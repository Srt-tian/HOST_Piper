...
bash "${RUNNER}" -c 'import torch,transformers,deepspeed; assert torch.__version__=="2.4.0+cu124"; assert transformers.__version__=="4.57.3"; assert deepspeed.__version__=="0.14.4"; assert torch.cuda.device_count()==4'
exec bash "${RUNNER}" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=4 --max_restarts=0 \
 ...
 --output /root/autodl-tmp/host_piper/runs/alignment_4task_weights_20260909 \
 --weights-only --checkpoint-interval 100 --keep-checkpoints 3 \
